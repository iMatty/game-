angular.module("gameDetailsFetcher")
    .factory("gameDetailsFetcher", ["$rootScope", "$q", "$http", "$filter", "steamApi",
        ...
        $rootScope.$watch(() => steamReady && galaxyReady,
                        (dataReady) => gameDetailsFetcher.dataReady = dataReady);

        let gameDetailsFetcher = {
           initialized: false,
           data: [],
           dataReady: true,

           fetchBySearchTerm: function(searchTerm) {
               reinitialize();
               getSteamDetailsBySearchTerm(searchTerm);
               getGogDetailsBySearchTerm(searchTerm);
           },

           fetchByIds: function(gogIds, steamIds) {
               reinitialize();
        ...
        function reinitialize() {
            ...
            gameDetailsFetcher.data = [];
            gameDetailsFetcher.dataReady = false;
        }
